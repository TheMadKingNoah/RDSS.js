...
export default class ModAlert {
    ...
    public static createModAlert(reaction: { emoji: { id: string; }; message: { fetch: () => Promise<any>; }; client: { channels: { cache: { get: (arg0: string) => any; }; }; }; }, user: { id: any; }){
        ...
        reaction.message.fetch().then( message => {
            ...
            if(channel !=null) {
                const newAlert = (channel as TextChannel).send({ content:
                    ...
                    , components: [row] });
                    Properties.existingModAlerts.set(message.id, message.id);
                }
            }    
        )
    }
